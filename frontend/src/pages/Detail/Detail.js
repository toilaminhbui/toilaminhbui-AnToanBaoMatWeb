...
import {
    capNhatBinhLuanPhimAction,
    layChiTietBinhLuanPhimAction,
    layDanhSachBinhLuanPhimAction,
    layThongTinPhimAction,
    themBinhLuanPhimAction,
    xoaBinhLuanPhimAction,
...
export default function Detail(props) {
    ...
    const { lichChieuTheoPhim } = useSelector(
    ...
    const [lichChieuTheoRap, setLichChieuTheoRap] = useState(
        lichChieuTheoPhim.filter((item) => item.ngayChieu === "")
    );
    const handleClick = (event) => {
        let clickNgayChieu = event.target.name;
        let lichChieuTheoRap = lichChieuTheoPhim.filter(
            (item) => item.ngayChieu === clickNgayChieu
        );
        setLichChieuTheoRap(lichChieuTheoRap);
    };
    let listNgayChieu = lichChieuTheoPhim
        .map((item) => item.ngayChieu)
        .filter((item, index, arr) => arr.indexOf(item) === index);
    let listTinhThanh = _.uniq(
        _.flattenDeep(
            lichChieuTheoRap.map((rapchieu, index) => {
                return rapchieu.rapchieu.map(
                    (tinhthanh, index) => tinhthanh.tinhthanh
                );
            })
        )
    );
    ...
    let uniqueTinhThanh = [
        ...new Map(
            listTinhThanh.map((item) => [item["maTinh"], item])
        ).values(),
    ];
    const noNullCumRap = _.uniqBy(
        _.flatten(lichChieuTheoRap.map((item) => item.rapchieu)),
        "maRap"
    );
    ...
    const handleClickRap = (event) => {
        ...
        let cumRaptheotinh = noNullCumRap.filter(
            (item) => item.maTinh_id === clickTinhThanh
        );
    ...
    const currentArrBinhLuan = reveseArrBinhLuanPhim.slice(
        indexOfFirstPost,
        indexOfLastPost
    );
            ...
            const content = (
                <div className="d-flex flex-col">
                    <Button
                        ...
                        onClick={() => {
                            dispatch(
                                layChiTietBinhLuanPhimAction(item.maComment)
                            );
                        }}
                    >
                        Sửa
            ...
            return (
                ...
                >
                    <div className="d-flex align-center">
                        {arrUser.find((us) => us.email == item.useremail)
                            ?.avatar !== null ? (
                            <div
                                style={{
                                    width: 40,
                            ...
                            <Avatar
                                ...
                                icon={item.username.substr(0, 1)}
                            />
                        )}
                        <div className="w-full">
                            <p className="my-auto m-3 text-danger">
                                {item.username}
                            </p>
                        ...
                        userLogin.role === "Super" ? (
                            <Popover
                                ...
                                trigger="hover"
                            >
                                <div className="btn cursor-pointer px-3 border-none drop-shadow-none hover:bg-gray-100">
                                    ...
                                </div>
                            </Popover>
                        ) : (
        ...
        <div style={{ position: "absolute", height: "auto", width: "100%" }}>
            ...
            <div className="row h-full relative z-10 mt-60">
                <div className="col-4 d-flex justify-end items-center h-full">
                    <img
                        className="posterphim mr-3 object-cover"
                        src={movieEditDetail.hinhAnh}
                        alt={movieEditDetail.hinhAnh}
                    />
                </div>
                <div className="col-4 d-flex justify-start items-center z-10">
                    <div className="text-white">
                        <p>
                            Ngày chiếu:{" "}
                            {moment(movieEditDetail.ngayKhoiChieu).format(
                                "DD.MM.YYYY"
                            )}
                        </p>
                ...
                <div className="col-4 d-flex justify-start items-center">
                    <div
                        className={`c100 p${
                            (movieEditDetail.danhGia / 10) * 100
                        } big green`}
                    >
            ...
            <div
                className="container px-5 pb-2 mb-5 rounded-2xl"
                style={{
                    minHeight: "500px",
            ...
            >
                ...
                >
                    <TabPane
                        tab={
                            <p className="text-lg bg-slate-800 px-5 py-2 rounded-full">
                                Lịch Chiếu
                            </p>
                        }
                        key="1"
                        ...
                        {listNgayChieuActive.sort().map((item, index) => {
                            return (
                                <button
                                    key={index}
                                    type="button"
                                    className="text-white mr-3 mt-3 bg-purple-700 hover:bg-purple-800 focus:outline-none focus:ring-4 focus:ring-purple-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mb-2"
                                    onClick={handleClick}
                                    name={item}
                                >
                                ...
                                <List.Item
                                    ...
                                    extra={
                                        <div style={{ minWidth: "80%" }}>
                                            {_.orderBy(lichChieuTheoRap, [
                                                "gioChieu",
                                            ])
                                                .filter(
                                                    (rap) =>
                                                        rap.maRap === item.maRap
                                                )
                                                .filter(
                                                    (rap) =>
                                                        rap.ngayChieu === today
                                                ...
                                                .map((item, index) => {
                                                    return (
                                                        <Tag
                                                            key={index}
                                                            disabled
                                                            className="text-lg mr-3 px-3 opacity-50 cursor-default select-none"
                                                            color="gray"
                                                        >
                                                            {item.gioChieu.substr(
                                                                0,
                                                                5
                                                            )}
                                                        </Tag>
                    ...
                    >
                        <p className="text-lg text-slate-900">
                            {movieEditDetail.moTa}
                        </p>
                    </TabPane>
                    <TabPane
                        tab={
                            <p className="text-lg bg-slate-900 px-5 py-2 rounded-full">
                                Đánh Giá
                            </p>
                        }
                        key="3"
                    >
                        <h1 className="mt-5 text-xl text-slate-900">
                        ...
                        <div className="bg-light rounded-xl p-2 mb-5 ">
                            {localStorage.getItem(TOKEN) ? (
                                <Form
                                    ...
                                    className="w-full d-flex flex-col items-end"
                                >
                                    ...
                                    <button
                                        ...
                                        type="submit"
                                        className="bg-blue-700 disabled:opacity-25 rounded-full text-white p-2 px-5"
                                    >
                                        Gửi
                                    </button>
                                </Form>
